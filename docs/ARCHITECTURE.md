...
2026-09-19 增补：应用通过独立 Supabase 项目的 `app-updates` Storage 桶读取版本清单与完整 APK；电脑不是业务服务器或更新下载源。云端业务数据库和 Edge Functions 与应用版本发布独立。

更新入口位于“我的 → 版本与更新”，不依赖登录。UI 通过更新 ViewModel 和 Repository 检查版本、下载及恢复状态；更新流程不读取或迁移业务记录。下载中断重新开始，已完成文件在恢复与安装前重新校验。安装交给系统确认，首次从没有 OTA 入口的 0.3.x 版本需同证书覆盖安装。

客户端固定信任项目 HTTPS 地址和发布证书，校验清单边界、版本、文件大小、摘要及 APK 实际身份；不会使用清单指定的新证书。安装包按 versionCode 使用不可覆盖路径，发布器核验公开文件后才提升最新清单。私钥和 Storage 写凭据仅位于 Secret；应用中只有公开读取地址。发布失败后的锁与重试见 [OTA 运维说明](OTA_OPERATIONS.md)，契约见 [清单 Schema](ota-manifest.schema.json)。
