...
最终完整回归集（含P4）在API26为43项，API33/35各42项，全部0失败、0跳过。通知权限拒绝后收件箱保留、设置返回状态刷新、真实系统通知去重/撤销、WorkManager账号切换取消、旧账号PendingIntent隔离及通知栏点击均通过。API26实际失败已由显式Closeable接口修复；旧版权限准备改为真实系统总开关，保留全部低版本用例。

本轮产品修复仅涉及通知权限状态与设置入口，以及通知库旧框架Closeable兼容；SQL、数据库版本和投递规则未改变。已查看API26设置关闭/返回开启、系统通知及详情截图，API33/35拒绝与返回状态。真机后台时效、厂商省电和重启仍待验收。
