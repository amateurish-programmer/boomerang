...
if [[ "$api" -ge 33 ]]; then
  ...
  bash android/gradlew -p android connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.class=$permission_class"
  permission_status=$?
...
bash android/gradlew -p android connectedDebugAndroidTest "-Pandroid.testInstrumentationRunnerArguments.notClass=$permission_class"
test_status=$?
...
adb pull /sdcard/Android/data/com.boomerang.app/files/acceptance/. android/app/build/reports/androidTests/acceptance/ >/dev/null 2>&1 || true
