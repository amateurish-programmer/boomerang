...
mkdir -p "$diagnostics"
adb shell dumpsys meminfo > "$diagnostics/memory-before.txt"
adb shell df -h > "$diagnostics/disk-before.txt"
...
test_status=$?
adb logcat -b events -d -s am_anr > "$diagnostics/anr-events.txt"
adb shell dumpsys meminfo > "$diagnostics/memory-after.txt"
adb shell dumpsys cpuinfo > "$diagnostics/cpu-after.txt"
